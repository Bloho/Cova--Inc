  ...
  return (
    <header className="shell app-header">
      ...
      <nav className="header-actions" aria-label="Primary">
        ...
        <Link className="icon-button" href="/?search=1" aria-label="Search movies" prefetch>
          <Search size={23} strokeWidth={2.4} />
        </Link>
        <Link className="icon-button" href={isSignedIn ? (username ? `/${username}` : "/") : "/login"} aria-label={displayName ?? "Account"} prefetch>
